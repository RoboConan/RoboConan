...
class SuiteSparseCamdConan(ConanFile):
    name = "suitesparse-camd"
    description = "CAMD: Routines for permuting sparse matrices prior to factorization in SuiteSparse"
    ...
    def package(self):
        copy(self, "License.txt", os.path.join(self.source_folder, "CAMD", "Doc"), os.path.join(self.package_folder, "licenses"))
        cmake = CMake(self)
        cmake.install()
    ...
    def package_info(self):
        self.cpp_info.set_property("cmake_file_name", "CAMD")
        self.cpp_info.set_property("cmake_target_name", "SuiteSparse::CAMD")
        if not self.options.shared:
            self.cpp_info.set_property("cmake_target_aliases", ["SuiteSparse::CAMD_static"])
        self.cpp_info.set_property("pkg_config_name", "CAMD")

        suffix = "_static" if is_msvc(self) and not self.options.shared else ""
        self.cpp_info.libs = ["camd" + suffix]
