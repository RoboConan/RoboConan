...
class GdalConan(ConanFile):
    ...
    def generate(self):
        ...
        tc.cache_variables["GDAL_USE_ARROWDATASET"] = (
        ...
        if self.options.with_arrow:
            ...
            tc.cache_variables["Parquet_FOUND"] = arrow.options.parquet
            tc.cache_variables["Parquet_VERSION"] = str(arrow.ref.version)
            tc.cache_variables["ArrowDataset_FOUND"] = arrow.options.dataset_modules
            tc.cache_variables["ArrowDataset_VERSION"] = str(arrow.ref.version)
    ...
    def package_info(self):
        ...
        if self.options.with_qhull:
            self.cpp_info.requires.extend(["qhull::libqhull_r"])
        if self.options.with_spatialite:
