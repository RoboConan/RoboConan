...
class SuiteSparseBtfConan(ConanFile):
    ...
    def package(self):
        copy(self, "License.txt", os.path.join(self.source_folder, "BTF", "Doc"), os.path.join(self.package_folder, "licenses"))
        copy(self, "lesser.txt", os.path.join(self.source_folder, "BTF", "Doc"), os.path.join(self.package_folder, "licenses"))
        cmake = CMake(self)
        cmake.install()
    ...
    def package_info(self):
        self.cpp_info.set_property("cmake_file_name", "BTF")
        self.cpp_info.set_property("cmake_target_name", "SuiteSparse::BTF")
        if not self.options.shared:
            self.cpp_info.set_property("cmake_target_aliases", ["SuiteSparse::BTF_static"])
        self.cpp_info.set_property("pkg_config_name", "BTF")

        suffix = "_static" if is_msvc(self) and not self.options.shared else ""
        self.cpp_info.libs = ["btf" + suffix]
