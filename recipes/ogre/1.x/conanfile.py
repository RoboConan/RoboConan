...
class OgreConanFile(ConanFile):
    ...
    def generate(self):
        ...
        tc.cache_variables["OGRE_STATIC"] = not self.options.shared
        tc.cache_variables["OGRE_RESOURCEMANAGER_STRICT"] = 2 if self.options.resourcemanager_strict == "STRICT" else 1
        tc.cache_variables["OGRE_BUILD_RENDERSYSTEM_D3D9"] = self.options.get_safe("build_rendersystem_d3d9", False)
        tc.cache_variables["OGRE_BUILD_RENDERSYSTEM_D3D11"] = self.options.get_safe("build_rendersystem_d3d11", False)
        tc.cache_variables["OGRE_BUILD_RENDERSYSTEM_GL3PLUS"] = self.options.get_safe("build_rendersystem_gl3plus", False)
        tc.cache_variables["OGRE_BUILD_RENDERSYSTEM_GL"] = self.options.get_safe("build_rendersystem_gl", False)
        tc.cache_variables["OGRE_BUILD_RENDERSYSTEM_GLES2"] = self.options.get_safe("build_rendersystem_gles2", False)
        tc.cache_variables["OGRE_BUILD_RENDERSYSTEM_METAL"] = self.options.get_safe("build_rendersystem_metal", False)
        tc.cache_variables["OGRE_BUILD_RENDERSYSTEM_TINY"] = self.options.get_safe("build_rendersystem_tiny", False)
        tc.cache_variables["OGRE_BUILD_RENDERSYSTEM_VULKAN"] = self.options.get_safe("build_rendersystem_vulkan", False)
        tc.cache_variables["OGRE_BUILD_COMPONENT_PAGING"] = self.options.build_component_paging
        tc.cache_variables["OGRE_BUILD_COMPONENT_MESHLODGENERATOR"] = self.options.build_component_meshlodgenerator
        tc.cache_variables["OGRE_BUILD_COMPONENT_TERRAIN"] = self.options.build_component_terrain
        tc.cache_variables["OGRE_BUILD_COMPONENT_VOLUME"] = self.options.build_component_volume
        tc.cache_variables["OGRE_BUILD_COMPONENT_PROPERTY"] = self.options.build_component_property
        tc.cache_variables["OGRE_BUILD_COMPONENT_OVERLAY"] = self.options.build_component_overlay
        tc.cache_variables["OGRE_BUILD_COMPONENT_OVERLAY_IMGUI"] = self.options.get_safe("build_component_overlay_imgui", False)
        tc.cache_variables["OGRE_BUILD_COMPONENT_BITES"] = self.options.get_safe("build_component_bites", False)
        tc.cache_variables["OGRE_BUILD_COMPONENT_BULLET"] = self.options.build_component_bullet
        tc.cache_variables["OGRE_BITES_STATIC_PLUGINS"] = self.options.get_safe("bites_static_plugins", False)
        tc.cache_variables["OGRE_BUILD_COMPONENT_PYTHON"] = False
        tc.cache_variables["OGRE_BUILD_COMPONENT_JAVA"] = False
        tc.cache_variables["OGRE_BUILD_COMPONENT_CSHARP"] = False
        tc.cache_variables["OGRE_BUILD_COMPONENT_RTSHADERSYSTEM"] = self.options.build_component_rtshadersystem
        tc.cache_variables["OGRE_BUILD_RTSHADERSYSTEM_SHADERS"] = self.options.get_safe("build_rtshadersystem_shaders", False)
        tc.cache_variables["OGRE_BUILD_SAMPLES"] = False
        tc.cache_variables["OGRE_BUILD_TOOLS"] = self.options.get_safe("build_tools", False)
        tc.cache_variables["OGRE_BUILD_XSIEXPORTER"] = self.options.get_safe("build_xsiexporter", False)
        tc.cache_variables["OGRE_BUILD_LIBS_AS_FRAMEWORKS"] = False  # TODO: requires additional package_info() logic
        tc.cache_variables["OGRE_BUILD_TESTS"] = False
        tc.cache_variables["OGRE_BUILD_PLUGIN_ASSIMP"] = self.options.build_plugin_assimp
        tc.cache_variables["OGRE_BUILD_PLUGIN_BSP"] = self.options.build_plugin_bsp
        tc.cache_variables["OGRE_BUILD_PLUGIN_GLSLANG"] = self.options.build_plugin_glslang
        tc.cache_variables["OGRE_BUILD_PLUGIN_OCTREE"] = self.options.build_plugin_octree
        tc.cache_variables["OGRE_BUILD_PLUGIN_PFX"] = self.options.build_plugin_pfx
        tc.cache_variables["OGRE_BUILD_PLUGIN_DOT_SCENE"] = self.options.build_plugin_dot_scene
        tc.cache_variables["OGRE_BUILD_PLUGIN_PCZ"] = self.options.build_plugin_pcz
        tc.cache_variables["OGRE_BUILD_PLUGIN_CG"] = False  # Cg is legacy and not worth adding support for on CCI, most likely
        tc.cache_variables["OGRE_BUILD_PLUGIN_EXRCODEC"] = self.options.build_plugin_exrcodec
        tc.cache_variables["OGRE_BUILD_PLUGIN_FREEIMAGE"] = self.options.build_plugin_freeimage
        tc.cache_variables["OGRE_BUILD_PLUGIN_RSIMAGE"] = False  # Requires Rust support on CCI
        tc.cache_variables["OGRE_BUILD_PLUGIN_STBI"] = self.options.build_plugin_stbi
        tc.cache_variables["OGRE_CONFIG_DOUBLE"] = self.options.config_double
        tc.cache_variables["OGRE_CONFIG_NODE_INHERIT_TRANSFORM"] = self.options.config_node_inherit_transform
        tc.cache_variables["OGRE_CONFIG_THREADS"] = "3" if self.options.config_threads else "0"
        tc.cache_variables["OGRE_CONFIG_ENABLE_MESHLOD"] = self.options.config_enable_meshlod
        tc.cache_variables["OGRE_CONFIG_ENABLE_DDS"] = self.options.config_enable_dds
        tc.cache_variables["OGRE_CONFIG_ENABLE_PVRTC"] = self.options.config_enable_pvrtc
        tc.cache_variables["OGRE_CONFIG_ENABLE_ETC"] = self.options.config_enable_etc
        tc.cache_variables["OGRE_CONFIG_ENABLE_ASTC"] = self.options.config_enable_astc
        tc.cache_variables["OGRE_CONFIG_ENABLE_QUAD_BUFFER_STEREO"] = self.options.get_safe("config_enable_quad_buffer_stereo", False)
        tc.cache_variables["OGRE_CONFIG_ENABLE_ZIP"] = True
        tc.cache_variables["OGRE_CONFIG_ENABLE_VIEWPORT_ORIENTATIONMODE"] = self.options.config_enable_viewport_orientationmode
        tc.cache_variables["OGRE_CONFIG_ENABLE_GLES2_CG_SUPPORT"] = False
        tc.cache_variables["OGRE_CONFIG_ENABLE_GLES2_GLSL_OPTIMISER"] = self.options.get_safe("config_enable_gles2_glsl_optimiser", False)
        tc.cache_variables["OGRE_CONFIG_ENABLE_GL_STATE_CACHE_SUPPORT"] = self.options.get_safe("config_enable_gl_state_cache_support", False)
        tc.cache_variables["OGRE_CONFIG_FILESYSTEM_UNICODE"] = self.options.get_safe("config_filesystem_unicode", False)
        tc.cache_variables["OGRE_CONFIG_STATIC_LINK_CRT"] = is_msvc_static_runtime(self)
        tc.cache_variables["OGRE_ENABLE_PRECOMPILED_HEADERS"] = False  # Fails for GCC in C3I with 'ADD_CUSTOM_COMMAND called with OUTPUT containing a ">"'
        tc.cache_variables["OGRE_INSTALL_SAMPLES"] = False
        tc.cache_variables["OGRE_INSTALL_TOOLS"] = self.options.get_safe("build_tools", False)
        tc.cache_variables["OGRE_INSTALL_DOCS"] = False
        tc.cache_variables["OGRE_INSTALL_PDB"] = False
        tc.cache_variables["OGRE_PROFILING"] = False
        tc.cache_variables["OGRE_LIB_DIRECTORY"] = "lib"
        tc.cache_variables["OGRE_BIN_DIRECTORY"] = "bin"
        tc.cache_variables["OGRE_INSTALL_VSPROPS"] = False
        # https://github.com/OGRECave/ogre/blob/v14.3.0/CMake/ConfigureBuild.cmake#L63-L69
        tc.cache_variables["OGRE_ASSERT_MODE"] = self.options.assert_mode
        tc.cache_variables["OGRE_BUILD_DEPENDENCIES"] = False
        # https://github.com/OGRECave/ogre/blob/v14.3.0/CMake/InstallResources.cmake
        tc.cache_variables["OGRE_PLUGINS_PATH"] = self._to_cmake_path(self._plugins_dir)
